...
- в интерфейсе можно временно переключать `OpenAI` и `Deepgram` для A/B теста без правки `.env`;
- значение `TRANSCRIBE_PROVIDER` остается серверным дефолтом, который UI подхватывает при загрузке.
